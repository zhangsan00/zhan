                         ...
                         <div class="am-u-sm-12 am-u-md-8">
                             <div class="am-btn-toolbar ">
                                 <div class="am-btn-group am-btn-group-xs">
                                     ...
                                     <a style="display: none;" id="sureFriend"><button   name="import" type="submit" class="am-btn am-btn-default am-btn-success"><span class="am-icon-pencil-square-o"></span>确认导入</button></a>
                                 </div>
                             </div>

                             <div style="font-size: 12px;color: red;padding-left: 105px;" >导入说明:仅支持txt文件格式,内容格式如下:友链排序|友链标题|友链地址URL  (两条友链之间用回车隔开)	</div>
                         </div>
                                    ...
                                    <tbody style="word-break: break-all;">	
										...
										if (!empty($AdminIeUrl)){
                                            ...
                                            for ($x=0; $x<=$count-1; $x++) {
                                                ...
                                                }else if ($TestIeUrlState =='ok'){
                                                    $IeUrlStateName='<a style="color: #00BCD4;">目标地址首页检测成功我方友链地址，状态正常</a>';
                                                }else {
                                                    $IeUrlStateName='<a style="color: #00BCD4;">未检测</a>';
                                                }
...
//导入友链
    ...
    {
        //解析文件友链
        $dataStr = file_get_contents($_FILES["friendLink"]["tmp_name"]);
            ...
            if (!empty($data)){
                foreach ($data as $k =>$v){
                    ...
                    if (is_array($item)&&count($item)==3){
                        ...
                        echo 'alertError("数据格式错误");';
                    }
                }
            }
        }else{
            echo 'alertError("无数据,请按照格式写入数据");';
        }
        ...
        echo'<script language="javascript">alert("文件格式错误"); </script>';exit();
    }
