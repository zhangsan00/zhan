...
$GongGao ='';
$gongaoFile ='./Plug/Plug_Gonggao/Plug_data/AdminGonggao.php';
//echo realpath('./Plug/Plug_Gonggao/Plug_data/AdminGonggao.php');
//exit;
if(file_exists('./Plug/Plug_Gonggao/Plug_data/AdminGonggao.php')){
    ...
    $AdminGonggao = json_decode(file_get_contents($gongaoFile),true);
    $WebGongao	=	$AdminGonggao['WebGongao'];
    $WebGongaoOpen	=	(int)$AdminGonggao['WebGongaoOpen'];
    ...
    if($WebGongaoOpen == 1){
        ...
        $GongGao .='<a href="javascript:void(0);" id="close1" class="close1"><img style="width: 409px;height: 255px" class="img" src="'.$WebGonggaoImg.'" /></a>';
//        $GongGao .='<a href="javascript:void(0);" id="close1" class="close1"><img class="img" src="/Static/Home/GongGao/img/welcom.png" /></a>';
        ...
        $GongGao .=$WebGongao;
        $GongGao .='</div>';
        $GongGao .='<a href="javascript:void(0);" class="button" id="copyUrl">复制本站网址</a>';
        $GongGao .='<!--<a href="https://down.gaobiaoiot.net/app/lilai_1.apk" class="button">下载安卓APP</a>-->';
        if (!empty($WebGonggaoApp)){
            $GongGao .='<a href="'.$WebGonggaoApp.'" class="button">下载安卓APP</a>';
        }
//        $GongGao .='<a href="javascript:void(0);" class="link">防封邮箱：'.$WebGonggaoEmail.'</a>';
        $GongGao .='</div>';
        $GongGao .='</div>';
